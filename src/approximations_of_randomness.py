...
def order_flip_sequence(flip_sequence):
    ...
    return divide_and_conquer_sort(new_list_flip_sequence) #make it so the number of heads is ordered
...
for person, flip_sequence in flips.items():
    ordered_random_flip_sequence.append([order_flip_sequence(flip_sequence).count(i) for i in range(0, 5)])

print('ordered_random_flip_sequence', ordered_random_flip_sequence)

diverged_flip_seqences = [kl_divergence([probability(j, 5) for j in range(0, 5)], ordered_random_flip_sequence[i]) for i in range(0, 5)]
print('Divergence')
