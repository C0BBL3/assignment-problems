...
class EulerEstimator:
	...
	def plot(self, x_range, stepsize=0.1, filename='plot.png'):
		plt.style.use('bmh')
		original_point = [self.point[0], self.point[1]]
		xs = [round(i * stepsize, 1) for i in range(int(x_range[0] * 1 / stepsize), int(x_range[1] * 1 / stepsize))]
		ys_1 = [self.step(stepsize, if_return_point=True) for x in xs[:xs.index(original_point[0])]]
		self.point = original_point
		ys_2 = [self.step(stepsize, if_return_point=True) for x in xs[xs.index(original_point[0]):]]
		self.point = original_point
