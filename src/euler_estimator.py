...
class EulerEstimator:
    ...
    def plot(self, x_range, stepsize=0.1, filename='plot.png'):
        ...
        xs = [i for i in range(x_range[0], x_range[1] + stepsize, stepsize)]
        ys_1 = [self.step(stepsize, if_return_point=True) for x in xs[:original_point[0]]] #its pronounced y's not yes i would like ice cream its y's plural y
