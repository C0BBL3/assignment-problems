    ...
    for num_2 in range(0, 10):
        for num_3 in range(0, 10):
            for num_4 in range(0, 10):
                ...
                filled_arr = [[arr[i][j] if j < max_index[1] else 15 - sum(arr[i]) for j in range(0, max_index[1] + 1)] if i < max_index[0] else [15 - sum([row[j] for row in arr]) if j < max_index[1] else 15 - sum(
                    [arr[i][j] for i in range(0, max_index[0]) for j in range(0, max_index[1]) if i == j]) for j in range(0, max_index[1] + 1)] for i in range(0, max_index[0] + 1)]
                ...
                if no_duplicate_entries(filled_arr) and is_valid(fixed_arr):
                    for row in fix_array(fixed_arr):
                        print(row)
                    exit()
